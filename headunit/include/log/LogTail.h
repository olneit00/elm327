...
// RadioService). Every mutation/read is guarded by a short FreeRTOS mutex.
#include <Arduino.h>
...
constexpr size_t kLogMaxLines   = 32;    // rolling line-cache capacity
constexpr size_t kLogLineLimit  = 96;    // max chars saved per line
...
private:
    ...
    // Dense rolling array of complete lines (contiguous, index 0 = oldest).
    // Heap-allocated: static DRAM BSS on the ESP32 is extremely tight, and a
    // multi-KB line cache there overflows dram0_0_seg at link time. The heap
    // segment has far more room, so we keep the cache out of static memory.
...
// calls (see git history for the sed that did it). "Serial" the object and
// Serial.begin() are intentionally left untouched so the physical UART and
// the AsyncLoggingMiddleware continue to work.
// ---------------------------------------------------------------------------
